...
# Optimized for use with version 3.5.50-st of the Wrapper.
#
...
APP_NAME="bamboo-agent"
APP_LONG_NAME="Bamboo Agent"
...
launchinternal() {
    ...
    then 
        ...
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" wrapper.daemonize=TRUE $APPNAMEPROP $ANCHORPROP $IGNOREPROP $STATUSPROP $COMMANDPROP $LOCKPROP wrapper.script.version=3.5.50 $ADDITIONAL_PARA"
        eval "$COMMAND_LINE"
    else
...
console() {
    ...
    then
        ...
        # The string passed to eval must handles spaces in paths correctly.
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" $APPNAMEPROP $ANCHORPROP $STATUSPROP $COMMANDPROP $LOCKPROP wrapper.script.version=3.5.50 $ADDITIONAL_PARA"
        eval "$COMMAND_LINE"
        COMMAND_EXIT_CODE=$?
